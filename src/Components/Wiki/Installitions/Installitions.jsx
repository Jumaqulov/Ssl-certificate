import React, { useEffect } from 'react'
import WikiCategories from '../WikiCategories'

export default function Installitions() {
    useEffect(() => {
        document.title = 'gogetssl.uz - Wiki - Руководства по установке'
    }, [])
    return (
        <>
            <div className="wiki-path">
                <a href="/">Главная</a>
                <span className='next'>
                    <svg viewBox='0 0 19 32'>
                        <path
                            d="M5.66962.995l12.304 12.89c.642.643.964 1.287.961 2.144s-.326 1.501-.97 2.144l-12.353 12.839c-.539.643-1.288.856-2.254.854-.858-.001-1.609-.218-2.25-.863-1.392-1.397-1.39-2.79.008-4.288l10.314-10.7-10.272-10.74c-1.391-1.505-1.39-2.9.009-4.289.645-.641 1.396-.854 2.253-.852.966 0 1.716.218 2.25.86v.001z">
                        </path>
                    </svg>
                </span>
                <a href='/wiki'>Wiki</a>
                <span className='next'>
                    <svg viewBox='0 0 19 32'>
                        <path
                            d="M5.66962.995l12.304 12.89c.642.643.964 1.287.961 2.144s-.326 1.501-.97 2.144l-12.353 12.839c-.539.643-1.288.856-2.254.854-.858-.001-1.609-.218-2.25-.863-1.392-1.397-1.39-2.79.008-4.288l10.314-10.7-10.272-10.74c-1.391-1.505-1.39-2.9.009-4.289.645-.641 1.396-.854 2.253-.852.966 0 1.716.218 2.25.86v.001z">
                        </path>
                    </svg>
                </span>
                <span className='current'>Руководства по установке</span>
            </div>
            <div className="wiki-main">
                <h3>Вики - Руководства по установке</h3>
                <p className='cert-txt-item'>
                    <span className='wiki-about-svg'>
                        <svg className='cert-txt-svg' viewBox="0 0 32 32" style={{ color: '#DC8510' }}>
                            <path
                                d="M31.875 14.25c0-.5-.563-.813-1-.813-1.5 0-2.75-.875-3.313-2.188-.563-1.375-.188-2.938.875-3.938.375-.313.375-.875.125-1.25a16.521 16.521 0 0 0-2.563-2.563c-.375-.25-.875-.25-1.188.125-.938 1.063-2.688 1.438-4 .875a3.549 3.549 0 0 1-2.188-3.438c.063-.5-.313-.875-.75-.938a11.72 11.72 0 0 0-3.625 0c-.438 0-.813.438-.75.875 0 1.5-.875 2.875-2.25 3.438-1.25.5-3 .125-3.938-.938a.87.87 0 0 0-1.188-.063c-1 .75-1.875 1.625-2.625 2.563-.25.375-.25.875.125 1.188 1.125 1 1.438 2.625.875 4-.563 1.313-1.875 2.188-3.438 2.188-.563-.063-.875.313-.938.75a11.72 11.72 0 0 0 0 3.625c0 .5.563.813 1 .813 1.438-.063 2.75.813 3.313 2.188s.188 2.938-.875 3.938c-.375.313-.375.875-.125 1.25.75.938 1.625 1.813 2.563 2.563.375.25.875.25 1.188-.125.938-1.063 2.688-1.438 4-.875a3.549 3.549 0 0 1 2.188 3.438c-.063.5.313.875.75.938.625.063 1.25.125 1.875.125.563 0 1.125-.063 1.75-.125.438 0 .813-.438.75-.875 0-1.5.875-2.875 2.188-3.438 1.375-.5 3.063-.125 4 .938a.87.87 0 0 0 1.188.063c1-.75 1.813-1.625 2.625-2.563.25-.375.25-.875-.125-1.188-1.125-1-1.438-2.625-.875-4 .5-1.313 1.813-2.188 3.25-2.188h.188c.5.063.875-.313.938-.75a11.72 11.72 0 0 0 0-3.625zM16 25.25c-5.125 0-9.25-4.125-9.25-9.25S10.875 6.75 16 6.75A9.23 9.23 0 0 1 25.25 16c0 1.75-.5 3.375-1.313 4.75l-4.063-4.063c.313-.688.438-1.438.438-2.188 0-1.5-.563-2.938-1.625-4s-2.5-1.688-4-1.688c-.5 0-1 .063-1.5.188a.716.716 0 0 0-.438.5.503.503 0 0 0 .188.563s2 2.063 2.625 2.688c.063.063.063.25.063.313v.063c-.063.688-.188 1.563-.313 1.938v.063h-.063c-.375.125-1.25.25-2 .313h-.063c-.063 0-.188 0-.313-.125-.688-.688-2.625-2.563-2.625-2.563a.541.541 0 0 0-.438-.25c-.25 0-.5.188-.625.5a5.79 5.79 0 0 0 1.5 5.5c1.063 1.063 2.5 1.625 4 1.625.75 0 1.5-.125 2.188-.438l4.063 4.063c-1.438.938-3.125 1.5-4.938 1.5z">
                            </path>
                        </svg>
                    </span>
                    <span>В данном разделе перечислены наиболее распространенные и популярные проблемы, связанные с SSL-сертификатами. У нас есть рабочие решения для большинства проблем, с которыми вы можете столкнуться при установке или использовании SSL в основных браузерах. Вы узнаете, как восстановить закрытый ключ или почему в браузере нет замка.</span>
                </p>
                <ul className='wiki-guide'>
                    <li>
                        <div className='wiki-guide-num'>
                            <span>1</span>
                        </div>
                        <div>
                            <a href="/wiki/installation/digicert-sectigo-ssl-installation">Руководства по установке DigiCert и Sectigo SSL</a>
                            <p>Наша команда подготовила список наиболее распространенных ресурсов, которые помогут вам сгенерировать CSR-код, установить SSL-сертификат на ваш сервер и проверить правильность установки...</p>
                        </div>
                    </li>
                    <li>
                        <div className='wiki-guide-num'>
                            <span>2</span>
                        </div>
                        <div>
                            <a href="/wiki/installation/redirect-http-to-https-in-nginx-apache">Как перенаправить HTTP на HTTPS в Nginx и Apache</a>
                            <p>Для того чтобы изменить URL страницы, отображаемой в результатах поиска, Google рекомендует использовать 301 Redirect. Это лучший способ убедиться, что поисковые системы и пользователи будут направлены на правильную страницу, а также защитить себя от потери трафика из поисковых систем...</p>
                        </div>
                    </li>
                    {/* <li>
                        <div className='wiki-guide-num'>
                            <span>3</span>
                        </div>
                        <div>
                            <a href="/wiki/installation/gogetssl-site-seal-installation">Установка печати сайта GoGetSSL</a>
                            <p>GoGetSSL - известный бренд на рынке SSL с положительной историей. В течение 10 лет мы действительно заботимся о наших клиентах и их безопасности. В настоящее время у нас более 80 000 клиентов в 223+ странах, защищающих более 2,5 миллионов доменных имен...</p>
                        </div>
                    </li> */}
                </ul>
            </div>
            <WikiCategories />
        </>
    )
}